...
import { SOLUTIONS, WEB_PLANS, formatPlanPrice, type WebPlan } from "@/lib/site-content";

const ICONS = { landing: LayoutTemplate, website: Globe, ecommerce: ShoppingBag } as const;

function ModalPlanCard({ plan }: { plan: WebPlan }) {
  return (
    <article className={`flex min-w-0 flex-col rounded-3xl p-[2px] transition duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-2 hover:shadow-2xl ${plan.highlighted ? "bg-sp-gradient-primary shadow-sp-card hover:brightness-105" : "bg-sp-violet/25 hover:bg-sp-gradient-primary"}`}>
      <div className="flex h-full min-h-[480px] flex-col rounded-[calc(1.5rem-1px)] bg-sp-white p-5 transition duration-500 sm:p-6">
        ...
        <ButtonLink href={`/?plan=${encodeURIComponent(plan.name)}#contacto`} variant="primary" className="mt-6 w-full border-0 !bg-sp-gradient-action shadow-sp-soft hover:-translate-y-1 hover:scale-[1.04] hover:shadow-2xl hover:brightness-110">Elegir este plan</ButtonLink>
      </div>
    </article>
  );
}
...
export function SolutionsSection() {
  const [selectedId, setSelectedId] = useState<(typeof SOLUTIONS)[number]["id"]>("landing");
  const [modalOpen, setModalOpen] = useState(false);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [visibleCount, setVisibleCount] = useState(1);
  const touchStartX = useRef<number | null>(null);
  const selected = SOLUTIONS.find((solution) => solution.id === selectedId) ?? SOLUTIONS[0];
  const category = selected.id === "landing" ? "Landing Page" : selected.id === "website" ? "Website" : "E-commerce";
  const startPlan = WEB_PLANS.find((plan) => plan.category === category && plan.name.toLowerCase().includes("start"));
  const categoryPlans = useMemo(() => WEB_PLANS.filter((plan) => plan.category === category), [category]);
  ...
  const openPlans = (id: (typeof SOLUTIONS)[number]["id"]) => {
    setSelectedId(id);
    setCurrentIndex(0);
    setModalOpen(true);
  };
          ...
          <Reveal className="rounded-3xl bg-sp-surface p-4 shadow-sp-soft sm:p-6">
            <div className="grid gap-3" role="tablist" aria-label="Soluciones digitales">
              {SOLUTIONS.map((solution) => {
                const SolutionIcon = ICONS[solution.id];
          ...
          <Reveal delay={0.08}>
            <article className="flex flex-col rounded-3xl bg-sp-gradient-primary p-[1px] shadow-sp-card">
              <div className="flex min-h-[420px] flex-col rounded-[calc(1.5rem-1px)] bg-sp-white p-6 sm:p-8">
                ...
                <p className="mt-6 text-3xl font-extrabold text-sp-ink">desde {startPlan.price} <span className="text-base font-semibold text-sp-muted">{startPlan.tax}</span></p><p className="mt-1 text-xs text-sp-muted">{startPlan.note}</p>
